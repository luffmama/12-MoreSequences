...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(100,100),
                rg.Point(98,100)]
    circle = rg.Circle(rg.Point(99,100),1)
...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window,circle,8,"pink")
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(pts)):
        line = rg.Line(pts[k],rg.Point(circle.center.x,circle.center.y))
        line.color = color
...
def run_test_polygon():
    ...
    polygon(window,circle,18,"green",1)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(pts)-1):
        ...
        line.thickness = thickness
        line.attach_to(window)
    lastline = rg.Line(pts[len(pts)-1],pts[0])
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(pts)):
        if k < len(pts)-hops_to_next_point:
            line = rg.Line(pts[k],pts[k+hops_to_next_point])
            line.color = color
            line.thickness = thickness
            line.attach_to(window)
        else:
            line1 = rg.Line(pts[k],pts[-(len(pts)-k-hops_to_next_point)])
            line1.color = color
